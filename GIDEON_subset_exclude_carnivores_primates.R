...
df_no = subset(df_no, Order != "CARNIVORA" &  Order != "PRIMATES")
...
GIDEON = subset(GIDEON, Order != "CARNIVORA" & Order != "PRIMATES")

save(GIDEON, file = "GIDEON.Rdata")
print("number of species / zoonosis records in GIDEON excluding primates and carnivores")
print(dim(GIDEON)[1])
...
names(df_out)[names(df_out)=="binomial"]="Spp"
...
#save(df_out)
animal_dx_parasites = df_out
save(animal_dx_parasites, file = "animal_dx_parasites.Rdata")
