...
In GIDEON, filter: Disease --> Agent --> bacterium. Copy diseases into google sheet GIDEON_bacterium_dx 
Subset GIDEON.Rdata with GIDEON_bacterium_dx so that GIDEON contains only bacterial diseases
```{r}
...
####Assemble data on primates (prim-zdx-parasites.xls, google sheet), carnivores (carnivore-zdx-parasites.csv, dropbox), other mammals (animal-dx-parasites_20190103.csv, google sheet).
...
Version using taxizedb. Commenting out because yields error: 
...
####Fix taxonomy in df_all.Rdata using ncbi_taxonomy.Rdata. Commenting out for now because still working on ncbi_taxonomy_read.R
```{r}
...
Comment out, use instead full taxonomy downloaded from NCBI
```{r parasite_zdx_ncbi}
# source("parasite_zdx_ncbi.R")
```

Subset df_all by bacterial diseases (excluding mammals with no diseases). Save df_all.Rdata.
